...
public class GDACSLoader extends SiteLoader {
    ...
    public String infoExtractor(String textFrom, String textTo, String content, String monthFormat, String dayFormat) {
        int indexOfSrt = content.indexOf(textFrom, content.indexOf("alert_box_title")); // Адрес текущего положения каретки при поиске (чтобы повторно не производить поиск в проверенном блоке0
...
//                name = tempName = "Заглавная_11111"; // для теста
            ...
            String dat = content.substring(indexOfSrt, content.indexOf(60, indexOfSrt));
            ...
            if (date[2].trim().equals(monthFormat) &&
                    (date[1].equals(dayFormat) || (Integer.parseInt(date[1]) == Integer.parseInt(dayFormat) - 1) &&
                                    (date[3].compareTo("06:00") > 0))) {
                contGDACS = contGDACS.concat(name + " " + magnitude + " " + alertDate + ",\n");
            } else {
//                contGDACS = contGDACS.concat(";");
                break;
            }
        }
        /**
        ...
        if (contGDACS.length() == 0) {
            ...
            contGDACS = contGDACS.substring(0, contGDACS.length() - 2).concat(";\n"); // знак ";" в конце перечисления
        }
        if (textFrom.compareTo("Tropical cyclones") != 0) {
            return "в\n".concat(contGDACS);
        }
